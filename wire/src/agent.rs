...
macro_rules! wire_id {
    ($(#[$doc:meta])* $name:ident) => {
        ...
        impl $name {
            ...
            pub const fn from_bytes(payload: [u8; 16]) -> Self {
                Self(u128::from_be_bytes(payload))
            }
        }
...
/// four-level address as fixed-width big-endian payload: `stream_id`, `topic_id`,
...
impl LogPosition {
    ...
    pub fn from_bytes(payload: [u8; LOG_POSITION_BYTES]) -> Self {
        let u32_at = |start: usize| {
            u32::from_be_bytes(payload[start..start + 4].try_into().expect("4-byte slice"))
        };
        Self {
            stream_id: u32_at(0),
            topic_id: u32_at(4),
            partition_id: u32_at(8),
            offset: u64::from_be_bytes(payload[12..20].try_into().expect("8-byte slice")),
        }
    }
}
...
// Display and FromStr are hand-written, not strum-derived, deliberately: the
// `Unrecognized(u8)` passthrough variant must format its carried code
// (`unrecognized-42`) and must never parse from text, a combination strum's
// per-variant attributes cannot express (`disabled` removes the variant from
// Display too and the derive panics on it). Every pure unit-word vocabulary
// (`AgentRunState`, `ForkKind`, `ContentType`, ...) derives strum instead.
...
/// `json` as [`ContentType::Json`](crate::content::ContentType::Json), and a
...
/// not scale across users that each own their own streams and topics.
///
...
pub struct Signature {
    ...
    #[serde(with = "crate::encoding::bin_bytes")]
    pub bytes: Vec<u8>,
    /// Out-of-band interpretation attributes the signature also covers, so an
    /// intermediary cannot flip the codec (`agdx.ct`) or wire version (`agdx.av`)
    /// on a signed record without invalidating it. Folded into the signed preimage
    /// when present. Skip-none, so a context-less signature stays byte-identical to
    /// a pre-context one (an older signer, or a body whose interpretation is fixed).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub struct SignatureContext {
    /// The `agdx.ct` content-type code the body is encoded with.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub content_type: Option<u8>,
    /// The `agdx.av` wire version the envelope is stamped with.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Metadata key: the declared classification of the data the operation touches.
/// Advisory unless the envelope is signed.
pub const METADATA_DATA_CLASSIFICATION: &str = "data_classification";
/// Metadata key: the task this operation serves. Advisory unless the envelope
/// is signed.
pub const METADATA_TASK_CONTEXT: &str = "task_context";
/// Metadata key: the session's declared intent. Advisory unless the envelope
/// is signed.
...
mod tests {
    ...
    fn given_a_schema_id_named_like_a_content_type_when_round_tripped_then_should_stay_a_schema_id()
    {
        ...
        // The tag disambiguates a schema id that happens to be a content-type
        // name: an untagged encoding would decode this as ContentType::Json.
        ...
        // A genuine content-type still round-trips as one.
