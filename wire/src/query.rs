...
pub struct Query {
    ...
    // Whether `page.total` must be an exact count (a `COUNT(*)` query).
    // Default false: the server fetches one extra row past `limit` as an
    // existence probe instead, cheaper for large result sets. `page.total` is
    // then absent and `page.has_more` remains exact.
    #[cfg_attr(feature = "builders", builder(default))]
    #[serde(default, skip_serializing_if = "is_false")]
    pub want_total: bool,
}
...
impl Page {
    ...
    pub fn at_least(&self, rows_on_page: usize) -> usize {
        self.offset + rows_on_page
    }
    ...
    pub fn total_pages(&self) -> Option<u64> {
        match (self.total, self.limit) {
            (Some(total), limit) if limit > 0 => Some(total.div_ceil(limit as u64)),
            _ => None,
        }
    }
}
...
pub struct Row {
    ...
    // Source stream and topic ids, paired with partition/offset to address the
    // origin log message.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
mod tests {
    ...
    fn given_a_page_when_computing_total_pages_then_should_divide_by_limit() {
        ...
        assert_eq!(page.at_least(3), 3, "offset 0 plus this page's rows");
