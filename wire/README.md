...
| `headers` | the `agdx.*` / `gen_ai.*` header dictionaries + header caps |
...
| `hello` | `HelloReply` / `OpVersions` (the capability-handshake body) + `BackendAnnounce` (the backend's capability announcement to the streaming server), incl. the `features` capability bitset (`feature::KV_CAS`/`READ_YOUR_WRITES`/`STRONG_CONSISTENCY`) |
...
| `http_client` | (feature `http-client`) a typed `/agdx/*` client over an injected `Transport`, owning routes, base64url, query strings, and the bare-`Ok`-or-`ErrorBody` unwrap |
