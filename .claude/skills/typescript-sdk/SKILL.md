...
Sessions mirror the Rust facade: `laser.sessions(options?)` -> `Sessions` (`create(id)`, `start()`, `open(conversation)`) -> `Session` with `append(kind, data)`, `context()` / `contextWith(policy)` -> `SessionTurn[]` (`kind` plus the `ContextMessage` with its `topic`), `memory(namespace?)` -> `ScopedMemory` (`search(query)`), `checkpoint()` -> `Checkpoint` (`toJSON` / `Checkpoint.fromJSON`), `turnsAt`, `turnsSince`, `stateAt`, `replay`. `SessionOptions` sets `stream`, a per-kind `topics` map (one topic per kind, `InvalidError` otherwise), `memoryNamespace`, `contextTurns`, `contextTokens`. Underneath: `Cursor.until(ends)` bounds a replay per partition, `Topic.partitionCount()` and `Topic.tailOffsets()` back checkpoint capture, `ContextAssemblerBuilder.fromCheckpoint` / `toCheckpoint` and the `from-checkpoint` / `at` replay bounds read around one, and a topic nobody has written yet assembles as empty. Memory mirrors the Rust topology. `laser.memory(namespace)` uses the default audit topic, `laser.memoryOnTopic(topic, stream?)` opens an isolated existing topic, and `laser.memoryTopic(topic).stream(name).partitions(n).ttl(milliseconds).build()` configures one with message expiry. `noExpiry()` writes zero expiry. `laser.context(conversation).memory(handle)` must retain the exact topic-backed handle instead of substituting another namespace.
...
Publish attempts default to 60 seconds with three retries and 250 ms exponential backoff capped at 30 seconds. Rust builder methods `publish_timeout`, `publish_max_retries`, and `publish_retry_backoff` match Python `Laser.connect` keywords `publish_timeout_ms`, `publish_max_retries`, and `publish_retry_backoff_ms`, and TypeScript builder methods `publishTimeout`, `publishMaxRetries`, and `publishRetryBackoff`. Explicit settings override `LASER_PUBLISH_TIMEOUT_MS`, `LASER_PUBLISH_MAX_RETRIES`, and `LASER_PUBLISH_RETRY_BACKOFF_MS`. Exhaustion returns an error for the application to handle, never a process exit. Preserve message identity and confirmed chunks on retries. See [publish recovery](../../../docs/publish-recovery.md). The transport in `foreign/typescript/src/iggy/apache-iggy.ts` keeps two invariants that the three-node reconnect test depends on: a socket swap the Apache Iggy client performs while following a leader move is not a lost connection, and only one publish attempt runs per connection at a time, because the client relocates by re-issuing every queued command and a second queued send makes it settle back on the metadata leader.
