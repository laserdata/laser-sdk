...
`laser.sessions(options?)` returns `Sessions`, with `create(id)`, `start()`, and `open(conversation)` methods that return a `Session`. `append(kind, data)` records a turn. `context()` and `contextWith(policy)` return `SessionTurn[]`. Each turn contains its `kind` and a `ContextMessage` with the source `topic`. `memory(namespace?)` returns `ScopedMemory`, which provides `search(query)`.

`checkpoint()` returns a `Checkpoint`, with `toJSON` and `Checkpoint.fromJSON` for persistence. `turnsAt`, `turnsSince`, `stateAt`, and `replay` read around its saved offsets. `SessionOptions` sets `stream`, `topics`, `memoryNamespace`, `contextTurns`, and `contextTokens`. Each kind needs a distinct topic in `topics`. Shared topics raise `InvalidError`.

`Cursor.until(ends)` bounds replay per partition. `Topic.partitionCount()` and `Topic.tailOffsets()` support checkpoint capture. `ContextAssemblerBuilder.fromCheckpoint` and `toCheckpoint` read around a checkpoint, as do the `from-checkpoint` and `at` replay bounds. A topic without records assembles as empty.
