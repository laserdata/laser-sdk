...
- The Python package tracks its own version, independent of the Rust workspace, so each language SDK releases on its own cadence. It currently sits at `0.0.1-rc.4` while binding the Rust `laser-sdk` at `=0.0.1-rc.5` (the dependency pin).
