...
Named fields allow additive optional growth when readers can safely ignore it. Changed meaning, removed fields, or new executor-dispatched grammar is breaking and requires a coordinated package release across every consumer. All shipped operation surfaces remain version 1 before the package line reaches 1.0.

Query, control, KV, fork, graph, checkpoint, agent, and other surfaces negotiate independently through `OpVersions`. A client checks the version associated with the command it is about to send. Destination and checkpoint commands use `versions.checkpoint`, not the query version.
