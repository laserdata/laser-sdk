...
export class ContractBuilder {
  ...
  async send(): Promise<Contract> {
    ...
    const resolved = await resolveContract(this.laser, this.router, this.route)
    ...
    try {
      ...
      if (this.expiryMs !== undefined) {
        command = command.withDeadlineMicros(BigInt(Date.now() + this.expiryMs) * 1000n)
      }
      await command.send()
      if (run !== undefined) {
...
export async function scatter(
  ...
  const report = await scatterReport(laser, source, selector, payload, inboxRoute, deadlineMs)
