...
// record's headers so recall reconstructs a memory item and narrows by
// scope. Absent on a generic key-value entry, which carries no scope.
...
// One stored entry: an arbitrary-bytes key and value plus optional expiry.
// `version` is the optimistic-concurrency token a `KvCas` precondition
// matches against; `0` means "unversioned" and is never a valid compare
// token. `scope` is present only on a memory read-view row. `source` is the
// origin log record this entry was folded from, since every managed write
// goes log-first through a mutation topic.
...
// The key decoded as UTF-8, or `undefined` when it is not valid UTF-8. Keys
// are arbitrary bytes, so a binary key has no string form.
...
// `cursor` is absent when the scan reached the end.
...
// Request to read the value at `key` in `namespace`. With `ifNoneMatch` set
// to a version, the read returns `KvOutcome.notModified` instead of the
// value when the live version matches (a conditional GET).
...
// Compare-and-swap write at `key` in `namespace`: apply `value` (with
// optional expiry) only if `expect` holds.
...
// while the task's fence sequence still equals `fenceToken` (the at-most-
// one effective-writer gate).
...
// Request to remove `key` from `namespace`. With `ifMatch` set to a
// version, the delete applies only when the live version matches.
...
// Request to test presence and read metadata without the value.
...
// Request to set, refresh, or clear a key's expiry in place. `expiresAtMicros`
// absent clears the expiry.
...
// Request to apply a merge patch to a structured value without transferring
// the whole object. With `ifMatch` set, the patch applies only at that
// version.
...
// Request to copy the value at `key` to `toKey` (optionally into another
// namespace via `toNamespace`) in a single backend transaction.
...
// Request to move the value at `key` to `toKey`: copy plus delete of the
...
// Request to release an advisory lease early, presenting the `leaseToken`
// the grant returned.
...
// Request to list every namespace that holds at least one entry for the
// caller. Carries no data beyond the op version, so there is no matching
// interface, just the encoder.
...
// namespace. `prefix` matches keys that start with it in byte order.
// `start`/`end` bound an inclusive-start, exclusive-end key range.
// `keyContains` keeps only keys that are valid UTF-8 and contain the
// substring. `conversation` narrows to rows a given conversation wrote (the
// memory read view stamps this; a generic kv row leaves it unset).
...
// bounds as a scan. With no bounds it clears the whole namespace. No
// `limit`/`cursor`, and expiry is ignored.
...
// The successful outcome of a KV command, shaped per op. Additive: an
...
// Why a key-value operation failed. Additive: an unrecognized variant
// decodes rather than throws.
export type KvError =
...
// The result of a key-value operation: `ok` with the outcome, or `err` with
