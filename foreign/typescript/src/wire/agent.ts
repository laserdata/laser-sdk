...
// What a message is. Closed by design: adding a kind is a wire version bump,
// an unknown kind must fail decode rather than flow misinterpreted.
...
// A producer-supplied business idempotency key: non-empty, at most
// MAX_IDEMPOTENCY_KEY_BYTES.
...
// An agent's identity: non-empty, bounded, control-character-free.
...
// A2A's task lifecycle, ridden as a u8 code. Unknown codes decode and pass
// through as non-terminal, never rejected.
...
// Why an agent operation failed. Unknown codes decode and pass through.
...
// Why a message was dead-lettered. Unknown codes decode and pass through.
...
// An advertised skill's health. Unknown codes decode and pass through.
export type Health =
...
// OTel-aligned token accounting (`gen_ai.usage.*`). Advisory analytics
// input, never enforcement-grade accounting.
...
// The structured body of a `kind = error` envelope.
...
// The agent-level dead-letter capsule: the poison message's log position,
// the reason, and the original payload verbatim (the encoded
// `AgentEnvelope`, byte-identical, so redrive is trivially correct).
...
// name or a registered writer-schema id. Externally tagged so a content-type
// name and a schema id, both strings, can never be confused for each other.
...
// The live presence an agent advertises in its connection metadata: which
// agent this connection is, and the inbox topic it currently consumes work
// on. Distinct from `AgentCard`: presence vanishes on disconnect, the card
// is durable, replayed capability.
...
// The claim-check capsule a `agdx.ct = ref` body carries: where the content
// lives, how big it is, and a digest, so any consumer verifies fetched
// bytes against the log without trusting the store.
...
// envelope body, so an intermediary cannot flip the codec or wire version
// on a signed record without invalidating it.
...
// A detached envelope signature: designed but dormant, an additive
// `signature` envelope field for the future opt-in per-agent key registry.
...
// Must-understand feature bits for `AgentEnvelope.mustUnderstand`. No bits
// are defined yet, so a current build understands none and only ever sees
// `mustUnderstand === 0n`.
...
// metadata rules. The keys are pinned so transcripts, bridges, and
// projections agree without per-app conventions.
...
// `metadata` is the AGDX-native extension slot, distinct from headers and
// `body`. Foreign metadata (A2A `metadata`, MCP `_meta`) never maps into
// it, it tunnels whole inside `body`.
...
// The per-kind validity matrix, ported rule for rule from `wire/src/agent.rs`.
// R = required, O = optional, X = invalid, mechanical per kind.
export function validateAgentEnvelope(envelope: AgentEnvelope): void {
  ...
  if (kind === AgentKind.Response) {
    // finish_reason is optional here, no check.
