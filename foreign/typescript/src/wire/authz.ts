...
// Whether a grant permits or forbids. `Deny` always wins over `Allow`. Rust
// carries no serde catch-all for this enum, so an unrecognized word is a
// decode failure, not a value.
...
// The managed surface a grant applies to. Additive: an unrecognized feature
// name (from a newer peer) decodes to `"unrecognized"` rather than failing
// the whole grant set, matching Rust's `#[serde(other)]` catch-all. An
// unrecognized capability matches no real request, so it is inert:
...
// The verb a grant permits, derived from the command code by `featureAction`.
// Additive like `Feature`: an unrecognized action name decodes to
// `"unrecognized"` rather than failing the whole grant set.
...
// How a `ResourcePattern` matches a request's resource selector. Rust carries
// no serde catch-all for this enum, so an unrecognized word is a decode
// failure, not a value.
...
// unkeyed request (`undefined`) matches only a whole-feature pattern.
...
// One capability grant: an effect on a `feature:action`, optionally scoped to
// a resource pattern.
...
// A named set of grants, bound to users. A user's effective capability is
// the union of the grants of every bound role, minus any matching deny.
...
// valid name is non-empty, at most `MAX_ROLE_NAME_BYTES` bytes, and made only
// of ASCII letters, digits, `-`, `_`, and `.`.
...
// The `(feature, action)` a managed command code authorizes against.
// `undefined` for a code with no capability semantics (hello, backend hello,
// client metadata, batch, and the authz band itself), which is gated another
// way.
...
// The `Feature`/`Action` vocabulary used by the coarse-capability bitmask,
// in the exact declaration order Rust's `VariantArray` derive assigns
// ordinals, since `actionIndex` depends on that order being stable.
...
// the stride of the shared coarse-capability bitmask layout (`actionIndex`).
export const ACTION_COUNT = ACTIONS.length

// The bit index of a `(feature, action)` in the coarse-capability bitmask, a
// pure function shared by every enforcer so the fork and the plane cannot
// drift.
...
// selector decoded from a request, or `undefined` for an unkeyed op.
...
// when both its own grants and the invoking user's grants permit it. The
// agent can never exceed the user who invoked it (permission intersection).
...
// Request the caller's own effective capabilities. Carries no data beyond
// the op version, so there is no matching interface, just the encoder.
...
// Request to list roles, optionally filtered. Absent filters list every
// role.
...
// Bind roles to a user (replace the user's whole role set). `expectRevision`
// is the compare-and-swap precondition: apply only if the binding's current
// revision equals this. Absent means an unconditional replace.
...
// Read the authorization change history for a subject, paged by revision.
...
// An authorization command failure. Additive: an unrecognized variant
...
// Reply to any authorization command, shaped per request. Additive: an
