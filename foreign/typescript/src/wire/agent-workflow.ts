...
// dimensions, each optional (absent is unbounded on that dimension).
...
// Submit a task to an agent or workflow (`AGDX_AGENT_SUBMIT`). `agentId` names
// the target, `runId` lets the caller assign the run id (else the backend
...
// A run's lifecycle state. A pinned, closed snake-case vocabulary: unlike
// `AgentError`/`AgentOutcome`, Rust's `AgentRunState` carries no serde
// catch-all variant, so an unrecognized word is a decode failure here too,
// not a passthrough value.
...
// A run's metadata, returned by submit, status, and list. `updatedAtMicros`
// is the time of the last state mark (equal to `createdAtMicros` until one
// lands), and `detail` is the terminal summary (an error message on
// `failed`, absent when clean). `cancelRequested` is the recorded cancel
// intent, not a state: the engine observes it at its next step boundary.
...
// List runs (`AGDX_AGENT_LIST`), filtered and paged. Every field but the op
// version is optional: an empty request lists the caller's runs from the
// newest. `limit` is clamped to `MAX_PAGE_SIZE` and `cursor` is the opaque
// continuation from the previous page, the kv scan pattern.
...
// One page of runs: the rows plus the opaque cursor for the next page,
...
// The successful outcome of an agent or workflow command, shaped per op. An
// outcome tag this build does not recognize decodes to `"unrecognized"`
// rather than throwing, since the enum is additive server-side.
...
// Why an agent or workflow control op failed. Additive like `AgentOutcome`:
// an unrecognized variant decodes rather than throws.
