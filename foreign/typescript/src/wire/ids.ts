...
// The causal locator (`cause_at`, dead-letter `source`) rides the wire as one
// opaque byte string so the envelope names no server. This is the Iggy
// binding's packing of it, four fields as fixed-width big-endian bytes.
...
// Every wire id is a distinct u128 newtype (RecordId, ConversationId,
// CorrelationId, ChannelId) so a caller cannot pass one where another is
// expected, even though all four share byte layout and display form. The
// brand type parameter, not the shared behavior, is what keeps them apart at
// compile time, mirroring Rust's `wire_id!` macro emitting one struct per
// name over shared codec logic.
