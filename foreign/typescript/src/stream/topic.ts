...
export class Topic {
  ...
  // Idempotent create-if-missing. Apache Iggy's own ensure command checks
  // existence before creating, so a second call against an already-created
  // topic is a no-op read, not a duplicate-creation error.
  async ensure(partitions: number = DEFAULT_PARTITIONS): Promise<void> {
    await this.observed("ensure", { partitions }, () =>
      this.transport.ensureTopic(this.streamName, this.name, partitions)
    )
  }

  // Raw bytes, balanced/key/partition routing, no schema. `publish()` is
  // the fluent builder over this same call for JSON/CBOR bodies.
  ...
  // Multiple raw messages in one round trip, sharing one routing decision.
  ...
  // Joins (creating if absent) a consumer group, which the server assigns
  // partitions from across every member polling under the same name.
  ...
  // An owned, client-side replay cursor across every partition, independent
  // of any server-tracked consumer offset.
