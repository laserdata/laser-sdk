...
// An owned, client-side replay position across every partition of a topic,
// independent of any server-tracked consumer offset. `poll()` returning an
// empty array means caught up, not closed: call it again later, or use
// `stream()` to poll on an interval automatically. Each partition starts
// from the beginning unless `fromOffsets` sets specific starting points.
export class Cursor {
