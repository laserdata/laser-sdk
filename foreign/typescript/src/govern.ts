...
interface ChainEntry {
  readonly lock: Mutex
  digest?: string
  touchedMicros: bigint
  users: number
}

const DEFAULT_GOVERNOR_RETENTION: GovernorRetention = {
  capacity: 4096,
  idleTtlMs: 3_600_000
}
...
export class GovernorState {
  ...
  private readonly chains = new Map<string, ChainEntry>()
  private readonly slotWaiters: (() => void)[] = []
  private readonly retention: GovernorRetention
  ...
  private async emitChained(
    ...
    const entry = await this.acquireChain(key)
    try {
      await entry.lock.runExclusive(async () => {
        ...
        await emit(evidence)
        entry.digest = evidence.receiptDigest
      })
    } finally {
      entry.users -= 1
      entry.touchedMicros = this.nowMicros()
      this.slotWaiters.shift()?.()
    }
  }

  private async acquireChain(key: string): Promise<ChainEntry> {
    for (;;) {
      const now = this.nowMicros()
      this.pruneChains(now)
      const retained = this.chains.get(key)
      if (retained !== undefined) {
        retained.users += 1
        return retained
      }
      while (this.chains.size >= this.retention.capacity) {
        let oldestKey: string | undefined
        let oldestTouched: bigint | undefined
        for (const [candidateKey, entry] of this.chains) {
          if (
            entry.users === 0 &&
            (oldestTouched === undefined || entry.touchedMicros < oldestTouched)
          ) {
            oldestKey = candidateKey
            oldestTouched = entry.touchedMicros
          }
        }
        if (oldestKey === undefined) break
        this.chains.delete(oldestKey)
      }
      if (this.chains.size < this.retention.capacity) {
...
export function decodePolicyEvidence(payload: Uint8Array): PolicyEvidence {
  ...
  return {
    ...
    decision: parseVerdictKind(field.requiredString(map, "decision", context), context),
    mode: parseGovernorMode(field.requiredString(map, "mode", context), context),
    kind: parseActionKind(field.requiredString(map, "kind", context), context),
    ...
    outcome: parseEvidenceOutcome(field.requiredString(map, "outcome", context), context),
...
const VERDICT_KINDS: ReadonlySet<string> = new Set<Verdict["kind"]>([
  "allow",
  "observe",
  "block",
  "step_up",
...
function parseVerdictKind(value: string, context: string): Verdict["kind"] {
  if (!VERDICT_KINDS.has(value)) {
    throw new CodecError(`\`${value}\` is not a recognized decision verdict`, context, "decision")
  }
  return value as Verdict["kind"]
}

const GOVERNOR_MODES: ReadonlySet<string> = new Set(Object.values(GovernorMode))

function parseGovernorMode(value: string, context: string): GovernorMode {
  if (!GOVERNOR_MODES.has(value)) {
    throw new CodecError(`\`${value}\` is not a recognized governor mode`, context, "mode")
  }
  return value as GovernorMode
}

const ACTION_KINDS: ReadonlySet<string> = new Set(Object.values(ActionKind))

function parseActionKind(value: string, context: string): ActionKind {
  if (!ACTION_KINDS.has(value)) {
    throw new CodecError(`\`${value}\` is not a recognized action kind`, context, "kind")
  }
  return value as ActionKind
}

const EVIDENCE_OUTCOMES: ReadonlySet<string> = new Set<PolicyEvidence["outcome"]>([
  "effected",
  "blocked",
  "step_up",
  "deferred"
...
function parseEvidenceOutcome(value: string, context: string): PolicyEvidence["outcome"] {
