...
void test("given_a_memory_topic_ttl_when_built_then_should_configure_microseconds", async () => {
      ...
      return {
        ...
        topic(topic: string) {
          return {
              ...
              partitions: number,
              options: { readonly messageExpiryMicros?: bigint } = {}
            ): Promise<void> {
              Object.assign(ensured, {
                stream,
                topic,
                partitions,
                messageExpiryMicros: options.messageExpiryMicros
              })
              return Promise.resolve()
            }
          }
  ...
  await new MemoryTopicBuilder(laser, "incidents").partitions(4).ttl(86_400_000).build()
