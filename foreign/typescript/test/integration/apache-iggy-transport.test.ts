...
void test("given_running_iggy_when_connecting_then_should_authenticate_before_returning", async () => {
...
void test("given_a_refused_connection_when_connecting_then_should_reject_with_a_transport_error", async () => {
  await assert.rejects(
    ApacheIggyTransport.connect("iggy:iggy@127.0.0.1:9?reconnection_retries=0"),
    { name: "TransportError" }
  )
})
