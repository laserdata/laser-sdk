...
pub fn authorize_edge(
    ...
    let claims = laser_sdk::edge_auth::EdgeClaims { audience, scopes };
    match laser_sdk::edge_auth::authorize_edge(&claims, &expected_audience, &required_scope) {
        Ok(()) => (true, None),
        Err(denial) => (false, denial.challenge()),
    }
}
