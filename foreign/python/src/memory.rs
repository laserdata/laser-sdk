...
type EmbedFuture = Pin<Box<dyn Future<Output = PyResult<Py<PyAny>>> + Send>>;
...
impl Embedder for PyEmbedder {
    async fn embed(&self, text: &str) -> Result<Vec<f32>, LaserError> {
        ...
        let call = Python::attach(|py| -> PyResult<EmbedCall> {
            let value = self.callback.bind(py).call1((text,))?;
            if let Ok(vector) = value.extract::<Vec<f32>>() {
                return Ok(EmbedCall::Ready(vector));
            }
            Ok(EmbedCall::Pending(Box::pin(into_future(value)?)))
        })
        .map_err(|error| LaserError::Handler(format!("embedder call failed: {error}")))?;
        match call {
            EmbedCall::Ready(vector) => Ok(vector),
            EmbedCall::Pending(future) => {
                let value = future.await.map_err(|error| {
                    LaserError::Handler(format!("embedder await failed: {error}"))
                })?;
