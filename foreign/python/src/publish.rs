...
impl PyLaser {
    ...
    fn publish(&self, topic: String, stream: Option<String>) -> PyPublish {
        PyPublish::new(self.inner.clone(), stream, topic)
    }

    /// Start a batch publish that flushes 1..N records in a single Iggy send.
    #[pyo3(signature = (topic, *, stream=None))]
    fn publish_batch(&self, topic: String, stream: Option<String>) -> PyBatchPublish {
        PyBatchPublish::new(self.inner.clone(), stream, topic)
    }
}
...
impl PyPublish {
    fn new(laser: Laser, stream: Option<String>, topic: String) -> Self {
        Self {
            laser,
            stream,
            topic,
    ...
    fn send<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyAny>> {
        ...
        future_into_py(py, async move {
            let mut request = match &stream {
                Some(stream) => laser.publish_on(stream, &topic),
                None => laser.publish(&topic),
            };
            ...
            request = match body {
                Body::Empty => request,
                Body::Bytes(bytes) => request.payload(bytes),
                Body::Raw {
                    bytes,
                    content_type,
                    schema_id,
                } => {
                    let request = request.raw_bytes(bytes, content_type);
                    match schema_id {
...
impl PyBatchPublish {
    ...
    fn send<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyAny>> {
        ...
        future_into_py(py, async move {
            let mut request = match &stream {
                Some(stream) => laser.publish_batch_on(stream, &topic),
                None => laser.publish_batch(&topic),
            };
            if inline {
                request = request.inline_payload();
            }
            ...
            for body in bodies {
                request = match body {
                    Body::Empty => request,
                    Body::Bytes(bytes) => request.add_payload(bytes),
                    Body::Raw {
                        bytes,
                        content_type,
                        schema_id: None,
                    } => request.add_raw_bytes(bytes, content_type),
                    Body::Raw {
                        bytes,
                        content_type,
                        schema_id: Some(schema_id),
