...
pub struct Record {
    ...
    // queries. A builder field so the setter is the unary `.inline_payload()`
    // opt-in, matching the accessor grammar (never `.thing(true)`).
    #[builder(field)]
    ...
    // Writer-schema id for a schema-first codec (Avro, Protobuf), stamped as
...
impl<S: record_builder::State> RecordBuilder<S> {
    ...
    /// Ask the projector to inline the payload bytes alongside the row in the
    /// query index (off by default). Unary opt-in.
