...
use laser_wire::agent::{
    ...
    OPERATION_QUARANTINE, OPERATION_UNQUARANTINE,
};
...
/// A folded presence and the authenticated `user_id` of the connection that
/// advertised it. The user_id is the principal the substrate authenticated (the
...
impl<'a> AgentRegistry<'a> {
    ...
    pub async fn refresh(&mut self, now_micros: u64) -> Result<usize, LaserError> {
        ...
        for message in &messages {
                ...
                || operation == Some(OPERATION_UNQUARANTINE)
            {
                ...
                // privileged write here. When a verifier is enrolled it must be
                // validly signed; otherwise the registry topic's write access
                // control is the gate. A forged or unsigned record is skipped.
                if !self.fact_authorized(&envelope) {
                    false
                } else if operation == Some(OPERATION_QUARANTINE) {
    ...
    /// that verifies against an enrolled key; without one, the registry topic's
    /// write access control is trusted as the gate.
    #[cfg(feature = "sign")]
    fn fact_authorized(&self, envelope: &AgentEnvelope) -> bool {
        match &self.verifier {
    ...
    fn fact_authorized(&self, _envelope: &AgentEnvelope) -> bool {
        true
    }
    ...
    pub fn inbox_for_principal(&self, agent: &AgentId, user_id: u32) -> Option<&str> {
    ...
    pub async fn refresh_presence(&mut self) -> Result<usize, LaserError> {
        ...
        for connection in connections {
            if let Some(bytes) = connection.metadata.as_deref() {
                ...
                apply_presence(&mut fresh, bytes, connection.user_id);
            }
        }
...
impl Laser {
    ...
    pub fn agent_registry(&self) -> Result<AgentRegistry<'_>, LaserError> {
        ...
        let (cards, quarantined, presence, offsets, presence_read_at_micros) = {
            let guard = cache
                .lock()
    ...
    pub async fn clear_presence(&self) -> Result<(), LaserError> {
        self.send_raw_with_response(AGDX_SET_CLIENT_METADATA_CODE, bytes::Bytes::new())
            .await?;
        Ok(())
    }
...
impl ClientMetadataRequest<'_> {
    ...
    /// Only return connections authenticated as `user_id`.
    ...
    pub async fn page(&self) -> Result<ClientMetadataPage, LaserError> {
        ...
        let bytes = self
            ...
            .await?;
        let list: ClientMetadataList = decode_named(&bytes)?;
    ...
    fn given_connection_metadata_when_folded_as_presence_then_should_key_by_agent_and_skip_non_presence()
     {
        ...
        assert_eq!(presence[&worker].user_id, Some(7));
