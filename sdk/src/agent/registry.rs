...
pub(crate) struct RegistryCache {
    ...
    applied_fact_order: VecDeque<(RecordId, u64)>,
}
...
impl<'a> AgentRegistry<'a> {
    ...
    pub async fn refresh(&mut self, now_micros: u64) -> Result<usize, LaserError> {
        let messages = self.cursor.poll_iggy().await?;
        let mut folded = 0;
        for (_, message) in &messages {
            let Ok(decoded) = crate::agent::consumer::decode_agent_record(
                message,
                laser_wire::agent::features::NONE,
            ) else {
                continue;
            };
                ...
                || operation == Some(OPERATION_UNQUARANTINE)
            {
                    ...
                    )
                {
                    ...
                    if let Some(id) = envelope.record {
                        ...
                        self.applied_fact_order
                            .push_back((id, decoded.observed_at_micros));
                    }
                ...
                apply_card(&mut self.cards, &envelope, decoded.observed_at_micros)
            };
            if applied {
                folded += 1;
            }
        ...
        let cutoff = now_micros.saturating_sub(APPLIED_FACT_TTL_MICROS);
        while self.applied_fact_order.len() > APPLIED_FACT_CAPACITY
            || self
                .applied_fact_order
                .front()
                .is_some_and(|(_, observed)| *observed < cutoff)
        {
            if let Some((id, _)) = self.applied_fact_order.pop_front() {
                self.applied_facts.remove(&id);
            }
        }
        if let Ok(mut cache) = self.cache.lock() {
            ...
            cache.applied_fact_order = self.applied_fact_order.clone();
    ...
    fn fact_authorized(
        ...
        context: Option<&laser_wire::agent::SignatureContext>,
        observed_at_micros: u64,
    ) -> bool {
        match &self.verifier {
            Some(registry) => context.is_some_and(|context| {
                registry
    ...
    fn fact_authorized(
        ...
        _context: Option<&laser_wire::agent::SignatureContext>,
        _observed_at_micros: u64,
    ) -> bool {
        true
    }
    ...
    ///
    /// Served by Iggy, so it returns a transport error against a
    /// Iggy server that lacks the command.
    #[cfg(feature = "query")]
    ...
    pub fn agent_registry(&self) -> Result<AgentRegistry<'_>, LaserError> {
        ...
        let (
            ...
            (
                ...
                guard.applied_fact_order.clone(),
