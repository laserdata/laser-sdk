...
impl Laser {
    ...
    /// publish position. Composes Iggy `get_consumer_offset`, where an
...
impl AgentReplyReader {
    ...
    fn verified_principal(
        ...
        match &self.verifier {
            Some(registry) => match context.and_then(|context| {
                registry
                ...
                Some(verified)
                    if self
                        .expected_signer
                        .as_deref()
                        .is_none_or(|expected| expected == verified.principal) =>
                {
                    Ok(Some(verified.principal))
                }
                Some(_) | None => Err(()),
            },
            None => Ok(None),
            ...
            for message in batch.messages {
                ...
                let signal = match &envelope {
                    ...
                    Some(agdx) if agdx.correlation == Some(correlation) => {
                        let Ok(verified_principal) = self.verified_principal(
                            agdx,
                            decoded.signature_context.as_ref(),
                            decoded.observed_at_micros,
                        ) else {
