...
impl Laser {
    ...
    /// `LASER_TLS_CERT=<path>` at any CA file to override it (a rotated CA
    /// included), or disable the check with `LASER_NO_TLS=1`. Any other host
    /// is left untouched: bring your own `tls=true&tls_ca_file=<path>` for a
    /// self-hosted deployment, the same knobs Apache Iggy's connection string
    /// always accepts. The scheme is optional (`Laser::connect` prepends
    /// `iggy://` if missing), so all of these work:
    ///
    /// ```no_run
    ...
    /// Connect to the documented local-dev default,
    /// `iggy://iggy:iggy@127.0.0.1:8090` (the stock Apache Iggy container's
    /// credentials and TCP port). For anything beyond a local server use
    /// [`connect`](Self::connect) or [`connect_env`](Self::connect_env).
    pub async fn local() -> Result<Self, LaserError> {
        Self::connect("iggy://iggy:iggy@127.0.0.1:8090").await
    }
