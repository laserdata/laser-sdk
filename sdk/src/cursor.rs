...
impl Laser {
    ...
    pub fn reader(&self, topic: impl Into<String>) -> Result<Cursor<'_>, LaserError> {
    ...
    ) -> Result<Cursor<'_>, LaserError> {
        Cursor::new(self, &stream.into(), &topic.into())
    }
}

/// A resumable reader over one topic. Build it with [`Laser::reader`].
pub struct Cursor<'a> {
    laser: &'a Laser,
    stream: Identifier,
    topic: Identifier,
...
impl<'a> Cursor<'a> {
    fn new(laser: &'a Laser, stream: &str, topic: &str) -> Result<Self, LaserError> {
        Ok(Self {
