...
impl Laser {
    ...
    pub fn with_governor(&self, governor: Arc<dyn ActionGovernor>, mode: GovernorMode) -> Self {
        self.with_governor_retention(governor, mode, GovernorRetention::default())
    }

    /// The retained evidence-chain policy variant of [`Self::with_governor`].
    /// Chain heads are process-local. A restart or retention eviction starts a
    /// new local chain for that conversation.
    #[must_use]
    pub fn with_governor_retention(
        &self,
        governor: Arc<dyn ActionGovernor>,
        mode: GovernorMode,
        retention: GovernorRetention,
    ) -> Self {
        let mut governed = self.clone();
        governed.governor = Some(Arc::new(GovernorState::with_retention(
...
pub(crate) struct GovernorState {
    ...
    chain: tokio::sync::Mutex<HashMap<Option<u128>, Arc<ChainEntry>>>,
    chain_released: Arc<tokio::sync::Notify>,
    retention: GovernorRetention,
}

struct ChainEntry {
    digest: tokio::sync::Mutex<Option<String>>,
    touched_micros: AtomicU64,
}
...
/// Process-local retention for governance digest-chain heads. Eviction or process restart starts a new local chain for that conversation.
#[derive(Clone, Copy, Debug)]
pub struct GovernorRetention {
    /// Maximum retained conversation heads.
    pub capacity: usize,
    /// Inactivity duration after which an unlocked head may be evicted.
    pub idle_ttl: std::time::Duration,
}

impl Default for GovernorRetention {
    fn default() -> Self {
        Self {
            capacity: 4_096,
            idle_ttl: std::time::Duration::from_secs(3_600),
        }
    }
}
...
impl GovernorState {
    pub(crate) fn new(governor: Arc<dyn ActionGovernor>, mode: GovernorMode) -> Self {
        Self::with_retention(governor, mode, GovernorRetention::default())
    }

    pub(crate) fn with_retention(
        governor: Arc<dyn ActionGovernor>,
        mode: GovernorMode,
        mut retention: GovernorRetention,
    ) -> Self {
        retention.capacity = retention.capacity.max(1);
        Self {
            governor,
            mode,
            ...
            chain: tokio::sync::Mutex::new(HashMap::new()),
            chain_released: Arc::new(tokio::sync::Notify::new()),
            retention,
        }
    }

    fn prune_chain(&self, chain: &mut HashMap<Option<u128>, Arc<ChainEntry>>, now_micros: u64) {
        let ttl_micros = u64::try_from(self.retention.idle_ttl.as_micros()).unwrap_or(u64::MAX);
        chain.retain(|_, entry| {
            Arc::strong_count(entry) > 1
                || now_micros.saturating_sub(entry.touched_micros.load(Ordering::Acquire))
                    < ttl_micros
        });
    }

    async fn acquire_chain(&self, chain_key: Option<u128>) -> ChainLease {
        loop {
            let released = self.chain_released.notified();
            {
                let now = now_micros();
                let mut chain = self.chain.lock().await;
                self.prune_chain(&mut chain, now);
                if let Some(entry) = chain.get(&chain_key) {
                    return ChainLease {
                        entry: Arc::clone(entry),
                        released: Arc::clone(&self.chain_released),
                    };
                }
                while chain.len() >= self.retention.capacity {
                    let oldest = chain
                        .iter()
                        .filter(|(_, entry)| Arc::strong_count(entry) == 1)
                        .min_by_key(|(_, entry)| entry.touched_micros.load(Ordering::Acquire))
                        .map(|(key, _)| *key);
                ...
                if chain.len() < self.retention.capacity {
                    let entry = Arc::new(ChainEntry {
                        digest: tokio::sync::Mutex::new(None),
                        touched_micros: AtomicU64::new(now),
                    });
                    chain.insert(chain_key, Arc::clone(&entry));
                    return ChainLease {
                        entry,
                        released: Arc::clone(&self.chain_released),
                    };
    ...
    async fn emit_chained_evidence(
        ...
        let slot = self.acquire_chain(chain_key).await;
        let mut previous = slot.entry.digest.lock().await;
...
mod tests {
    ...
    #[tokio::test]
    async fn given_a_full_chain_table_when_a_slot_is_released_then_should_admit_without_growing() {
        let state = Arc::new(GovernorState::with_retention(
            Arc::new(AllowAll),
            GovernorMode::Enforce,
            GovernorRetention {
                capacity: 1,
        ...
        let first = state.acquire_chain(Some(1)).await;
        let waiting_state = Arc::clone(&state);
        let waiting = tokio::spawn(async move { waiting_state.acquire_chain(Some(2)).await });
        ...
        drop(first);
        let second = waiting.await.expect("chain admission task succeeds");
        assert_eq!(state.chain.lock().await.len(), 1);
        assert!(state.chain.lock().await.contains_key(&Some(2)));
        drop(second);
    }

    #[tokio::test]
    async fn given_an_expired_chain_head_when_reacquired_then_should_start_a_new_local_chain() {
        let state = GovernorState::with_retention(
            Arc::new(AllowAll),
            GovernorMode::Enforce,
            GovernorRetention {
                capacity: 1,
        ...
        let first = state.acquire_chain(Some(1)).await;
        *first.entry.digest.lock().await = Some("old-head".to_owned());
        drop(first);

        let second = state.acquire_chain(Some(1)).await;
        assert_eq!(*second.entry.digest.lock().await, None);
    }
