...
```toml
[dependencies]
laser-sdk = "=0.0.1-rc.17" # typed streaming plus provenance
# Add only the layers the application uses:
laser-sdk = { version = "=0.0.1-rc.17", features = ["agent", "managed"] }
```
...
let api_calls = laser.topic("api_calls").json::<ApiCall>();
...
This example uses only the default `streaming` and `provenance` features and runs against any Apache Iggy. `Laser::connect_env()` reads the same `LASER_CONNECTION_STRING` / `LASER_STREAM` variables the deployment guides and examples use, and `Laser::local()` targets the stock local container, so the connect line never needs editing between environments.
...
let api_calls = laser.topic("api_calls");
...
let orders = laser.topic("orders").json::<Order>(); // or .cbor::<Order>()
...
laser.topic("intents").json::<Intent>().publish(&intent)?.send().await?;
...
| --- | --- | --- |
| `laser.topic(name)` | a topic on the default stream | publish, batch, replay, ensure, the typed handle (`.json::<T>()` / `.cbor::<T>()` / `.schema::<T>(id)`), the raw Iggy producer/consumer front door |
...
The default stream is pinned at connect (`connect_with_stream`) and `laser.topic(name)` is the one-word shortcut against it. Without one it returns the typed `NoStream` error. Iggy RBAC is enforced at the stream and topic level, so a permission miss surfaces as a typed error (`is_permission_denied()` / `is_stream_or_topic_not_found()`), never a silent wrong answer.
