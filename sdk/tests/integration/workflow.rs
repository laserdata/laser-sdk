...
// Each capability-bearing worker opens its own connection: presence is
// connection-scoped and one connection advertises one agent.
async fn worker(laser: &Laser, id: &str, skill: &str, tag: &str) -> AgentHandle {
...
async fn wait_for_capable(laser: &Laser, skill: &str, expected: usize) {
    ...
    harness::eventually(|| {
        ...
        async move {
            ...
            (registry.resolve(&skill, now).len() == expected).then_some(())
        }
    })
    .await;
}
...
async fn given_concurrent_card_publishers_when_registry_refreshes_then_should_observe_every_card() {
    let laser = harness::laser().await;
    let publisher_a = harness::reconnect(&laser).await;
    let publisher_b = harness::reconnect(&laser).await;
    let card_a = card("publish-a");
    let card_b = card("publish-b");

    let (published_a, published_b) = tokio::join!(
        publisher_a.publish_card("publisher-a".parse().unwrap(), &card_a),
        publisher_b.publish_card("publisher-b".parse().unwrap(), &card_b),
    );
    published_a.expect("publisher a writes its card");
    published_b.expect("publisher b writes its card");

    wait_for_capable(&laser, "publish-a", 1).await;
    wait_for_capable(&laser, "publish-b", 1).await;
}
...
async fn given_a_two_step_workflow_when_run_then_should_dispatch_in_dependency_order_and_thread_outputs()
 {
    let laser = harness::laser().await;
    let mut triager = worker(&laser, "triager", "triage", "triaged").await;
    let mut diagnoser = worker(&laser, "diagnoser", "diagnose", "diagnosed").await;
    triager.ready().await.expect("triager joins its group");
    diagnoser.ready().await.expect("diagnoser joins its group");
    wait_for_capable(&laser, "triage", 1).await;
    wait_for_capable(&laser, "diagnose", 1).await;
...
async fn given_an_all_capable_step_when_run_then_should_scatter_and_fold_every_reply() {
    let laser = harness::laser().await;
    let mut reviewer_a = worker(&laser, "rev-a", "review", "a-reviewed").await;
    let mut reviewer_b = worker(&laser, "rev-b", "review", "b-reviewed").await;
    reviewer_a
        .ready()
        .await
    ...
    wait_for_capable(&laser, "review", 2).await;
...
impl AgentHandler for SagaWorker {
    async fn handle(&self, message: &AgentMessage, ctx: &AgentCtx<'_>) -> Result<(), LaserError> {
        if message.body() == b"undo" {
            self.compensated.fetch_add(1, Ordering::SeqCst);
            return ctx.respond(Bytes::from_static(b"undone")).await;
        }
        self.dispatched.fetch_add(1, Ordering::SeqCst);
        ctx.respond(Bytes::from_static(b"done")).await
    }
}

#[tokio::test]
#[serial_test::serial(integration)]
async fn given_a_budget_breach_after_a_completed_step_when_the_workflow_fails_then_should_run_compensation()
 {
    let laser = harness::laser().await;
    ...
    let result = laser
        ...
        .step(
            ...
            |_ctx: &StepContext<'_>| Bytes::from_static(b"charge").to_vec(),
        )
        .compensate_with(|_ctx: &StepContext<'_>| Bytes::from_static(b"undo").to_vec())
        .step(
            "ship",
            Router::to_capable("saga", RoutePolicy::Any),
            |_ctx: &StepContext<'_>| Bytes::from_static(b"ship").to_vec(),
        )
        .after("charge")
        .run()
        .await;

    assert!(matches!(result, Err(LaserError::BudgetExceeded { .. })));
