...
async fn given_an_agent_and_bridge_on_a_custom_stream_when_used_then_should_run_on_that_stream() {
    ...
    // multi-stream topologies and per-stream Iggy RBAC. `with_stream` is a cheap
    ...
    let default_stream = laser.stream().expect("the test laser has a default stream");
    let scoped = laser.with_stream(format!("{default_stream}-scoped"));
    scoped
        .bootstrap(2)
        .await
