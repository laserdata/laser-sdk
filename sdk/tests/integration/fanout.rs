...
async fn given_agents_advertising_a_capability_when_the_orchestrator_fans_out_then_should_route_to_each_and_gather_every_reply()
 {
    ...
    for id in worker_ids {
        ...
        // so capability routing discovers the worker. Its own connection:
        // presence is connection-scoped, one advertised agent per connection.
    ...
    // since Apache Iggy harness has no presence command), where each
