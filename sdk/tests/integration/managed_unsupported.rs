// The managed coordination surface against Apache Iggy (no managed backend):
...
use crate::harness::{client, laser};
use laser_sdk::prelude::full::{Capabilities, EdgeDir, GraphEdge, GraphNode, Laser};
...
// The refresh path re-probes the live connection and shares the outcome with
// every clone: a long-lived client discovers backend changes without being
// reconstructed, and an open host never flips a surface on.
#[tokio::test]
#[serial_test::serial(integration)]
async fn given_open_iggy_when_capabilities_are_refreshed_then_should_stay_open_across_clones() {
    let laser = laser().await;
    let clone = laser.clone();
    assert!(!laser.capabilities().await.managed);
    let refreshed = laser.refresh_capabilities().await;
    assert!(!refreshed.managed, "a refresh cannot invent a managed host");
    assert!(!refreshed.kv.cas);
    let shared = clone.capabilities().await;
    assert!(
        !shared.managed,
        "clones read the same negotiated state the refresh wrote"
    );
}

#[tokio::test]
#[serial_test::serial(integration)]
async fn given_builder_capabilities_when_refreshed_then_should_preserve_the_configured_seed() {
    let seeded = Laser::builder()
        .client(client().await)
        .capabilities(Capabilities::OPEN.with_query(true))
        .build()
        .await
        .expect("the seeded client should build");

    let refreshed = seeded.refresh_capabilities().await;

    assert!(
        refreshed.query.available,
        "refresh must preserve builder-supplied capabilities"
    );
    assert!(!refreshed.managed, "open Apache Iggy remains unmanaged");
}
