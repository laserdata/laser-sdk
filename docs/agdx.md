...
| `kv.lease` / `release` | state | advisory lease on a key | roadmap |
...
| --- | --- | --- |
| `kv.get` | namespace, key | `Value(Option<entry>)` |
...
| `kv.delete` | namespace, key | `Deleted(bool)` |
...
Errors: unsupported, invalid key, too large, backend, version, version-conflict. Namespaces isolate keys, scope scans, and separate one user's keys from another's. The trusted user id is stamped by the binding, never by the SDK. Scan caps: page at most 1000, default 100.
...
### A10.4 Proposed: advisory lease (roadmap)

```
kv.lease { namespace, key, lease_ttl_micros } -> { lease_token, granted_ttl_micros }
kv.release { namespace, key, lease_token }
```

A holder presents the token on protected mutations. A lease not renewed before its TTL expires is released automatically, and a stale token fails with `lease-lost`. Renewal and fencing semantics are still open and likely to change.
...
| Type | Fields |
| --- | --- |
| `Projection` | id (recommended `name.vN`), name, version, content-type, extraction schema, inline-payload default |
...
The control envelope carries `{ v, timestamp_micros, command }`. The command is one of `RegisterProjection`, `DropProjection`, `ApplyBinding`, `RemoveBinding`, `RegisterSchema`, `DropSchema`. Schema ids are permanent, collisions are rejected, and a dropped schema still decodes records already stamped with its id.
...
- At connect the client runs the `hello` operation. The reply advertises per-surface op versions (`query`, `control`, `kv`, `fork`, and an optional `agent` version where 0 means not advertised) plus an optional `features` bitset for managed sub-capabilities that vary independently of the base surface: compare-and-swap, read-your-writes, and strong consistency. A `0` bitset (skipped on the wire) means none advertised, so a pre-feature reply stays byte-identical and an old client simply lights up no extra capability.
- The SDK capability set distinguishes managed services served off the log (query, key-value, memory) from infrastructure-native capabilities (sessions, forks, durable dedup), and surfaces the feature bits as flags (`kv_cas`, `read_your_writes`, `strong_consistency`) so a client can check before attempting the op. On an open substrate with no managed surface every flag is false and the matching call returns unsupported. The HTTP surface carries the same information as the boolean flags on its capabilities reply (B4).
...
- Every extended feature defaults to **not advertised**: the HTTP `Capabilities` constructor leaves `kv_cas`/`read_your_writes`/`strong_consistency` off and a server opts each on only when it serves it, mirroring the skip-when-zero `features` bitset. A backend that advertised a feature it cannot honor would turn the clean unsupported error into a silent wrong answer, so the safe default is off and opt-in.
- When the streaming server and the managed backend are separate processes, the backend is the single source of its own capability truth. On connect it announces its `OpVersions` (versions plus feature bits) to the streaming server over their private socket as a `BackendAnnounce` (`AGDX_BACKEND_HELLO_CODE`), and the streaming server caches and relays it verbatim when it answers a client `hello`. This is what keeps the two carriages honest without the streaming server hardcoding bits the backend may or may not serve, and a backend that gains a feature lights it up everywhere by announcing one bitset.
...
The capabilities reply carries the per-surface booleans plus the three extended-feature flags (`kv_cas`, `read_your_writes`, `strong_consistency`). These default **off** and a server advertises one only when it genuinely serves it (A12): over-advertising would turn a clean unsupported error into a silent wrong answer.
...
| advisory lease and release (A10.4) | a bounded-TTL lease with a token, renewal and fencing still open, built on the shipped compare-and-swap |
