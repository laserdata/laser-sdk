...
A graph projection is registered and dropped through the control commands `RegisterGraph` / `DropGraph` on the control topic (A11.2), the same durable path as a row projection, not a dedicated HTTP route. The HTTP graph surface is the query and neighbor reads above. The `/graphs` path constants in the wire crate are reserved for a future console register surface and are not served today.
