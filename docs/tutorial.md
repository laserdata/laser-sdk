...
| memory | `LogMemory` (folds the log) + `VectorMemory<E>` (in-memory cosine recall, bring your own `Embedder`) | `QueryMemory<E>` (durable vector recall via `managed_query`) + `KvMemory` (durable point-state via `managed_kv`), surfaced through `Capabilities::managed_memory` / `managed_query` / `managed_kv` |
