...
> _With `.index_only()`, `fetch_typed::<T>()` cannot decode rows because their `payload` is `None`. Callers either use `.fetch()` and decode indexed columns directly off `Row.headers` / `Row.metadata`, or replay from the Iggy log. Plan the trade-off when you declare the projection, not at query time._
