...
Feature: Provenance and causality on the wire
  ...
  Scenario: A correlation id survives a round trip
    When I send an agent command "ping" with agent "planner" and correlation id "corr-1"
    And I assemble the conversation
    Then the assembled message payload is "ping"
    And the assembled message correlation id is "corr-1"
    And the assembled message belongs to the conversation
