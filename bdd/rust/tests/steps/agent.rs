...
async fn build_must_understand_event(world: &mut LaserWorld) {
    ...
    let bytes = encode_named(&envelope).expect("the marked envelope encodes");
    let back: AgentEnvelope = decode_named(&bytes).expect("the marked envelope decodes");
...
async fn build_plain_event(world: &mut LaserWorld) {
    ...
    let bytes = encode_named(&envelope).expect("the envelope encodes");
    let back: AgentEnvelope = decode_named(&bytes).expect("the envelope decodes");
    world.must_understand_unmet = Some(back.unmet_requirements(RECEIVER_UNDERSTOOD) != 0);
}
