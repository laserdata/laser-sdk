...
sys.path.insert(0, str(REPOSITORY_ROOT / "foreign" / "python"))
...
def iggy_endpoint():
    configured = os.environ.get("LASER_BDD_URL")
    if configured:
        yield configured
        return
    address = os.environ.get("LASER_BDD_ADDR")
    if address:
        yield f"iggy:iggy@{address}"
        return
...
class World:
    ...
    def send_command(self, payload, agent=None, idempotency_key=None, correlation_id=None):
...
@when(
    parsers.parse(
        'I send an agent command "{payload}" with agent "{agent}" '
        'and correlation id "{correlation}"'
    )
)
def send_command_with_agent_and_correlation(world, payload, agent, correlation):
    world.send_command(payload, agent=agent, correlation_id=correlation)
