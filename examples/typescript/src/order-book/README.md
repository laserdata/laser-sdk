...
The JSON tape is body-first. Projection pointers extract queryable scalars from the fill itself, and no duplicate index headers can disagree with that body. Because each tape record opts into inline payload, a query with payload selection, returns the original JSON bytes.
