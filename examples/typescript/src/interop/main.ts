...
import { runExample, utf8 } from "../common.js"
...
export async function run(laser: Laser, _signal: AbortSignal): Promise<void> {
  ...
  const handles = [
    worker(laser, "assistant", AgentTopic.Commands, AgentTopic.Responses),
    worker(laser, "tool-runner", AgentTopic.ToolCalls, AgentTopic.ToolResults),
    Agent.builder()
