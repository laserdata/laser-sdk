...
| [`kv`](src/kv/README.md) | State | set/get keyed JSON with a TTL, upgrade it under compare-and-swap, write and promote a fork row (managed) | `cargo run --example kv` | [`/laser-sdk/state`](https://docs.laserdata.cloud/laser-sdk/state) |
...
`recall` is the Memory primitive's example - it is not named `memory` because that name already belongs to the full scenario below (one of Memory's four verbs instead).
