...
For a LaserData host (`*.laserdata.cloud`) TLS and the CA cert attach automatically, even when you pass only the connection string. The public CA lives in `../certs/` (examples-level, shared across language ports). Point `LASER_TLS_CERT=<path>` at any CA file to override, the same knob as the connection string's `tls_ca_file=`. A string that already sets `tls_ca_file=`, a non-LaserData host, or `LASER_NO_TLS=1` is left untouched.
...
| [`memory-bench`](src/memory-bench/README.md) | agentic | the memory benchmark harness: a LongMemEval-shaped protocol (pinned needle facts under `LASER_MEMORY_BENCH_DISTRACTORS` distractor turns, one question per needle) scoring each recall strategy on needle-in-context accuracy, recall tokens, and latency, with the answer step on the `LlmClient` seam (deterministic `MockLlm` by default). A local run scores the deterministic stand-in, so wire a real embedder and model against a deployment to measure your own recall |
