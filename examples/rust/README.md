...
| [`recall`](src/recall/README.md) | agentic | an agent that learns from feedback: the four agentic-memory verbs as one loop. The assistant remembers what it knows, recalls the semantically closest facts for a question, improves the ranking from an operator upvote so the helpful fact rises next time, and forgets a superseded fact. Runs the whole loop in process over `VectorMemory`, so it needs no server |
