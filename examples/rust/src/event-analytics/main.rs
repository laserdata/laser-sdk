...
async fn main() -> Result<(), LaserError> {
    ...
    let projector = if query_available {
        ...
        cloud_feature_ready(false, "projection-backed analytics", "event-analytics");
        None
    };
    let publisher = {
...
// Total events, on the shared volume knob so one run scales from a smoke test
// to millions (`LASER_MESSAGES`).
...
async fn publish_clickstream(laser: &Laser, events: &[Event]) -> Result<(), LaserError> {
    ...
    for chunk in events.chunks(publish_chunk()) {
        let clicks = laser.topic(TOPIC);
        let mut batch = clicks.publish_batch();
        for event in chunk {
...
// Per-send chunk, on the shared volume knob (`LASER_BATCH`).
...
// poll (`CommitPolicy::Polling`), the one commit mode that keeps the server's
// stored offset moving in lockstep with delivery, so a redelivered or
// re-polled batch can never starve the reader.
...
async fn wait_for_projection(laser: &Laser, expected: usize) -> Result<(), LaserError> {
    ...
    // count only when the plane reported the view advanced. Elsewhere (the local
    // managed change feed) every tick queries, the plain bounded poll.
    let mut feed = laser
        .capabilities()
        .await
        .watch
    ...
    loop {
        let advanced = match &mut feed {
            Some(feed) => !feed.poll().await?.is_empty(),
            None => true,
        };
        ...
        if Instant::now() >= deadline {
            return Err(LaserError::Invalid(format!(
                "projector indexed only {last}/{expected} events before the deadline",
                last = if last == usize::MAX { 0 } else { last },
            )));
        }
...
async fn run_resumable_export(
    ...
    let events = laser.topic(TOPIC);
    let mut reader = events.replay()?;
...
async fn run_guarded_ingest(laser: &Laser) -> Result<(), LaserError> {
    ...
    let guarded = laser.topic(GUARDED_TOPIC);
    guarded
        .publish()
        .json(&valid)?
        .schema_id(schema_id)
        .send()
        .await?;
    // Malformed: `latency_ms` is a string, violating the schema. It decodes as
    // JSON fine, only the validation catches it.
    guarded
        .publish()
        .raw_bytes(
