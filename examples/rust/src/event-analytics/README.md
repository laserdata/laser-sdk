...
1. **Hot path.** A consumer-group reader tails the raw log live while the producer streams in chunks, folding a rolling ops ticker (events seen, checkouts). Offsets commit server-side on each poll (`AutoCommit::When(PollingMessages)`), so the stored offset moves in lockstep with delivery and the reader can never starve on a re-polled batch. A timeout turns any stall into a diagnostic instead of a hang.
...
Indexing is body-first: the projection's pointers extract every column out of the decoded JSON event, typed, so the index and the event body can never disagree and no `agdx.idx.*` headers duplicate the payload. Every record carries the `message_type` + `ts` convention fields so the reserved columns fill and the query sugar works. Point it at a deployment and the analytics read off the log through LaserData Cloud instead of the in-process projector, no code change.
...
- `laser.iggy_consumer_group(..)` with server-side commit-on-poll, the consumer configuration that stays correct under redelivery.
- `laser.publish_batch(topic)` chunked indexed publishing (each chunk one `send_messages` call, spread across partitions by the balanced partitioner).
