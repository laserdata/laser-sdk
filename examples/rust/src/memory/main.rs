...
//
// The first half needs no server. The durable-memory and graph halves require
...
async fn main() -> Result<(), LaserError> {
    ...
    // PART 2: DURABLE MEMORY AND THE KNOWLEDGE GRAPH. Connect after the local
    // phase, then use the server's advertised capabilities.
