...
//
// Indexes materialize only once a LaserData Cloud consumes the projection
// commands. On a raw Apache Iggy server with no managed backend the publish path
...
async fn main() -> Result<(), LaserError> {
    ...
    // query phases, while the raw streaming load works against any Apache Iggy.
    ...
    if config.register && query_available {
        ...
        info!("projection registration needs LaserData Cloud, publishing to the raw log only");
    }
    ...
    if config.query && query_available {
        ...
        info!("sample analytics needs LaserData Cloud, streaming run completed without it");
    }
...
// come from the example crate (`env_usize` and friends), never redefined here.
struct Config {
    orgs: usize,
    messages: u64,
    payload_bytes: usize,
...
async fn produce_shard(
    ...
    while published_in_shard < count {
        ...
        let firehose = laser.topic(topic);
        let mut batch = firehose.publish_batch();
