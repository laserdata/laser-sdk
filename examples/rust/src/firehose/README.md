...
Approximate log volume is `LASER_FIREHOSE_MESSAGES` times `LASER_FIREHOSE_PAYLOAD_BYTES`. The defaults publish 2M messages at 4 KB, roughly 8 GB across 8 org indexes. Indexes are created by LaserData Cloud consuming the projection commands, so point the example at a deployment and set `LASER_LOCAL_WORKER=0` for the indexes and trailing queries. On a raw Apache Iggy server the publish path still runs at full speed and the queries are skipped.
...
- `laser.publish_batch(topic)` with `LASER_FIREHOSE_CONCURRENCY` parallel producers and the balanced partitioner, the high-throughput indexed-publish path.
