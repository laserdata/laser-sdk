...
const LOCAL_CONNECTION_STRING: &str = "iggy:iggy@127.0.0.1:8090";
const DEFAULT_TCP_PORT: u16 = 8090;

/// Connect the way every example does. Resolves a target from the environment,
/// so the same binary runs against local Apache Iggy, Laser Stack, or a
/// LaserData Cloud deployment with no code change:
///
/// - `LASER_CONNECTION_STRING`: a bare `user:pwd@host` or `<token>@host` target. The SDK adds the transport scheme, and the port defaults to 8090 when omitted.
...
/// - neither set: local default (`iggy:iggy@127.0.0.1:8090`).
///
...
/// advertise only managed surfaces whose backend is ready, while Apache Iggy
/// keeps the supplied open capability set. A long-lived client can re-probe with
/// `refresh_capabilities` without losing the supplied set.
...
pub fn managed_feature_ready(enabled: bool, feature: &str, example: &str) -> bool {
    if enabled {
        return true;
    }
    println!(
        "{feature} is unavailable on this deployment.\n\
         ...
         LASER_CONNECTION_STRING=user:password@host cargo run --example {example}\n"
    );
    false
}
...
/// A projector handle for the query examples. Query is managed by `laser-plane`,
/// so the projection is registered on Laser Stack or LaserData Cloud. Returned
/// by [`start_projector`].
...
/// Register `topic`'s projection over `fields` on a managed deployment so
/// `query(topic)` returns rows. This requires Laser Stack or LaserData Cloud.
/// On Apache Iggy without `laser-plane`, registration and query both return
...
pub async fn start_projector(
    ...
    ensure_view(laser, topic, topic, content_type, fields).await?;
    Ok(Projector)
}
...
pub async fn ensure_view(
    ...
    // Wait until laser-plane has applied the registration and created the index
    // before we return, so records published next flow into a live
    ...
    while laser.query(index).fetch().await.is_err() {
        if Instant::now() >= deadline {
            return Err(LaserError::Invalid(format!(
                "laser-plane did not create index `{index}` in time (is the managed deployment running and consuming `control.commands`?)"
            )));
        }
...
/// Wait until `index` holds at least `expected` rows, so a query that follows
/// never races the projector. Where the deployment publishes the change feed
/// this awaits the advance and only then counts, the same await-then-query
/// shape the change-feed example teaches. Returns the final total.
pub async fn wait_for_rows(laser: &Laser, index: &str, expected: u64) -> Result<u64, LaserError> {
    let deadline = Instant::now() + Duration::from_secs(30);
    let mut feed = match laser.capabilities().await.watch {
        true => Some(laser.watch().index(index).records()?),
        false => None,
    };
    let mut last = None;
    loop {
        let advanced = match feed.as_mut() {
            Some(feed) => last.is_none() || !feed.poll().await?.is_empty(),
            None => true,
        };
        if advanced {
            let total = laser
                ...
                .fetch()
                .await
                .map(|result| result.page.total.unwrap_or_default())
                .unwrap_or_default();
            if last != Some(total) {
                println!("  projector materialized {total}/{expected} rows");
                last = Some(total);
            }
            if total >= expected {
                return Ok(total);
        ...
        if Instant::now() >= deadline {
            return Err(LaserError::Invalid(format!(
                "projector indexed only {}/{expected} rows in `{index}` before the deadline",
