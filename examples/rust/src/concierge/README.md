...
The queryable world model, KV, approvals, and forks run on LaserData Cloud. On raw Apache Iggy the example prints how to point at a deployment and exits green before provisioning the desk. The in-process `VectorMemory` remains useful independently in the `memory` and `memory-bench` examples.
