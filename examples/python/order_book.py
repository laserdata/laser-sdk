...
consume them:
  ...
  - the ANALYTICS path: the same fills are indexed to a queryable tape the
    ...
    drains. The query phase needs Laser Stack or LaserData Cloud and skips on
    Apache Iggy without a managed backend.
...
AVRO_TAPE_TOPIC = "trades_avro"  # schema-first tape (managed deployment)
