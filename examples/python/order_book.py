...
async def stream_live_book(laser: ls.Laser, trades: list[dict]) -> Book:
    ...
    async def feed() -> None:
        for start in range(0, len(trades), BURST):
            batch = laser.publish_batch(FEED_TOPIC)
    ...
    cursor = laser.reader(FEED_TOPIC)
    book = Book()
    seen = 0
    idle = 0.0
...
async def index_tape(laser: ls.Laser, trades: list[dict]) -> None:
    ...
    for start in range(0, len(trades), TAPE_BATCH):
        ...
        batch = laser.publish_batch(TAPE_TOPIC).inline_payload()
        for fill in chunk:
            batch = batch.add_json(fill)
        await batch.send()
        indexed += len(chunk)
...
async def avro_tape(laser: ls.Laser, trades: list[dict]) -> None:
    ...
    await laser.ensure_topic(AVRO_TAPE_TOPIC, partitions=_common.PARTITIONS)
    ...
    batch = laser.publish_batch(AVRO_TAPE_TOPIC).inline_payload()
    for fill in fills:
...
async def main() -> None:
    ...
    await laser.ensure_topic(FEED_TOPIC, partitions=_common.PARTITIONS)
    await laser.ensure_topic(TAPE_TOPIC, partitions=_common.PARTITIONS)
