...
Pass a connection target through the environment. The port defaults to 8090 when omitted. TLS and the CA cert attach automatically for a `*.laserdata.cloud` host (the public CA vendored in `../certs/`). Point `LASER_TLS_CERT=<path>` at any CA file to override, the same knob as the connection string's `tls_ca_file=`.
...
| [`memory_bench.py`](memory_bench.py) | agentic | the memory benchmark harness, peer of the Rust `memory-bench`: pinned needle facts under `LASER_MEMORY_BENCH_DISTRACTORS` distractor turns, one question per needle, each recall strategy scored on needle-in-context accuracy, recall tokens, and latency. A local run scores a deterministic stand-in embedder, so point at a deployment with a real embedding model to measure your own recall |
