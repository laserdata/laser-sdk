...
| [`memory.py`](memory.py) | agentic | agentic memory, both halves, peer of the Rust `memory`: the four memory verbs as one loop over a vector memory (remember, recall the semantically closest, improve from an operator upvote, forget a superseded fact), then the durable knowledge graph over the same ops domain (upsert services and components, read a node's neighbors, traverse from every `Service` to what it depends on). The graph half skips cleanly on raw Apache Iggy and the named graph is browsable in the console's graph explorer (AGDX A13) |
