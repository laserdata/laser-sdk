...
The first facet needs no server. The durable-memory and graph facets require
Laser Stack or LaserData Cloud.

Run it:
    LASER_CONNECTION_STRING=iggy:laser@127.0.0.1:8090 python3 memory.py
"""
...
async def main() -> None:
    ...
    if not (os.getenv("LASER_CONNECTION_STRING") or os.getenv("LASER_SERVER")):
        print("\nSet LASER_CONNECTION_STRING to use durable memory and the knowledge graph.")
        return
