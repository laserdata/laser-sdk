...
The first facet needs no server. The durable-memory and graph facets are
managed: against raw Apache Iggy they are skipped with a note. The durable
memories show in the console's Memory view and the named graph in its explorer.
...
async def main() -> None:
    ...
    # PART 1: MEMORY, in process with no server.
    await run_memory(conversation)

    if not (os.getenv("LASER_CONNECTION_STRING") or os.getenv("LASER_SERVER")):
        print(
    ...
    try:
        ...
        _common.managed_gate(False, "durable memory and the knowledge graph", "memory")
        return
...
def embed(text: str) -> list[float]:
    ...
    for token in text.lower().split():
        ...
        if token:
            vector[_token_hash(token.lower()) % DIMS] += 1.0
