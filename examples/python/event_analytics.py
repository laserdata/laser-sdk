...
VISITORS = ["alice", "bob", "carol", "dave", "erin", "frank", "grace", "heidi", "ivan", "judy"]
ROUTES = ["/home", "/product/42", "/product/7", "/search", "/cart", "/checkout", "/pricing"]
...
async def run_resumable_export(laser: ls.Laser) -> None:
    ...
    resumed = laser.reader(TOPIC, from_offsets=saved)
...
async def run_guarded_ingest(laser: ls.Laser) -> None:
    ...
    await laser.ensure_topic(GUARDED_TOPIC, partitions=_common.PARTITIONS)
    ...
    await laser.publish(GUARDED_TOPIC).json(valid).inline_payload().schema_id(schema_id).send()
    ...
    await laser.publish(GUARDED_TOPIC).json(malformed).inline_payload().schema_id(schema_id).send()
    ...
    settled = (await laser.query(GUARDED_TOPIC).fetch()).total
    if settled == 1:
        print(
...
async def main() -> None:
    ...
    await laser.ensure_topic(TOPIC, partitions=_common.PARTITIONS)
