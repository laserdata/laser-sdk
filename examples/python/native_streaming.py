...
async def main() -> None:
    ...
    print("producer: exact-width header, keyed routing, and 1000 batched messages")
    await publish_messages(producer)

    print("consumer: production interval-or-each auto commit")
    await receive_all(
    ...
    print("consumer: commit after successful handling (one round-trip per message)")
    await receive_all(
        topic.consumer_group(
            "manual-workers",
            batch_length=BATCH,
