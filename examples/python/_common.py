...
# examples. A ``.sandbox`` / ``.dev`` host uses the dev CA, every other LaserData
# host the prod CA.
CERTS = Path(__file__).resolve().parent.parent / "certs"
PROD_CERT = CERTS / "laserdata.crt"
DEV_CERT = CERTS / "laserdata_dev.crt"
...
def _is_laserdata_host(host: str) -> bool:
    return host.endswith(".laserdata.cloud") or host.endswith(".laserdata.com")


def _host_of(connection_string: str) -> str:
    authority = connection_string.split("://", 1)[-1]
    authority = authority.split("/", 1)[0].split("?", 1)[0]
    host_and_port = authority.rsplit("@", 1)[-1]
    return host_and_port.rsplit(":", 1)[0]
...
def _with_resolved_tls(connection_string: str) -> str:
    ...
    if _env("LASER_NO_TLS") or not _is_laserdata_host(host) or "tls_ca_file=" in connection_string:
        return connection_string
    cert = DEV_CERT if (".sandbox" in host or ".dev" in host) else PROD_CERT
...
async def start_projector(laser, topic, fields, *, content_type="json") -> None:
    ...
    await laser.apply_binding(
        {
            "source": {"stream": laser.stream, "topic": topic},
...
async def wait_for_projection(laser, topic, expected) -> int:
    """Poll until the projector has materialized at least `expected` rows in
    ...
    while True:
        ...
        if time.monotonic() >= deadline:
            raise ls.InvalidError(
                f"projector indexed only {total}/{expected} rows in '{topic}' before the deadline"
            )
        await asyncio.sleep(PROJECTION_POLL)
