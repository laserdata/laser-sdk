...
_RUN_TOKEN = f"{os.getpid() & 0xFFFF:04x}{time.time_ns() & 0xFFFF:04x}"


def run_token() -> str:
    """The per-invocation suffix shared by this run's stream and index names."""
    return _RUN_TOKEN
...
def managed_gate(available: bool, feature: str, example: str) -> bool:
    ...
    print(
        ...
        f"  Use Laser Stack or LaserData Cloud:\n"
        f"    LASER_CONNECTION_STRING=user:password@host python3 {example}.py\n"
    )
    return False
...
async def start_projector(laser, topic, fields, *, index=None, content_type="json") -> None:
...
async def wait_for_projection(laser, index, expected) -> int:
    """Wait until the projector has materialized at least `expected` rows in
    `index`, tolerant of a not-yet-created index. Returns the final total.
    ...
    deadline = time.monotonic() + PROJECTOR_TIMEOUT
    feed = laser.watch(index=index) if (await laser.capabilities()).watch else None
    last = -1
    while True:
