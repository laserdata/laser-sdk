...
LOCAL_CONNECTION_STRING = "iggy://iggy:iggy@127.0.0.1:8090"
DEFAULT_PORT = 8090
...
# The CA bundle is language-agnostic and lives one level up, shared with the Rust
# examples. LASER_TLS_CERT overrides it with any CA file path, the same knob the
# connection string's tls_ca_file= exposes.
CERTS = Path(__file__).resolve().parent.parent / "certs"
PROD_CERT = CERTS / "laserdata.crt"
...
def _is_laserdata_host(host: str) -> bool:
    # Mirrors the Rust example crate's `is_laserdata_host`: only the real
    # LaserData Cloud API domain, never the `laserdata.com` marketing site,
    # and the trailing-dot match rejects look-alikes like
    # "laserdata.cloud.attacker.com".
    host = host.lower()
    return host == "laserdata.cloud" or host.endswith(".laserdata.cloud")


def _host_of(connection_string: str) -> str:
    value = connection_string if "://" in connection_string else f"//{connection_string}"
...
def _ensure_default_port(connection_string: str) -> str:
    ...
    if urlsplit(connection_string).port is not None:
        return connection_string
    return f"{scheme}://{user_info}{host_and_port}:{DEFAULT_PORT}{path_and_query}"


def _with_resolved_tls(connection_string: str) -> str:
    if "://" not in connection_string:
        connection_string = f"iggy+tcp://{connection_string}"
    connection_string = _ensure_default_port(connection_string)
    host = _host_of(connection_string)
    if _env("LASER_NO_TLS") or not _is_laserdata_host(host) or "tls_ca_file=" in connection_string:
        return connection_string
    cert = _env("LASER_TLS_CERT") or PROD_CERT
    separator = "&" if "?" in connection_string else "?"
    tls = "" if "tls=" in connection_string else f"{separator}tls=true"
    glue = "&" if (tls or "?" in connection_string) else "?"
    return f"{connection_string}{tls}{glue}tls_ca_file={cert}"
...
def _resolve_connection_string() -> str:
    ...
    if provided:
        return _with_resolved_tls(provided)
    server = _env("LASER_SERVER")
    if not server:
        return LOCAL_CONNECTION_STRING
    return _with_resolved_tls(f"iggy+tcp://{_resolve_credentials()}{server}")
...
def managed_gate(available: bool, feature: str, example: str) -> bool:
    ...
    print(
        ...
        f"    LASER_CONNECTION_STRING=iggy://user:pwd@your-host python {example}.py\n"
    )
    return False
