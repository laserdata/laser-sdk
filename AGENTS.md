...
- Query is LaserData Cloud only: it rides the `AGDX_QUERY` managed command off the log (`send_raw_with_response`, no reply topic, no correlation poll), and against raw Apache Iggy returns `LaserError::Unsupported`. There is no topic request/reply query path. The connect-time `AGDX_HELLO` probe sets `managed_host` **and** implies `managed_query`, so a plain `Laser::connect(..)` against LaserData Cloud queries with no manual caps.
...
These early features exist to exercise the **seams** the paid tiers plug into. Their premium forms are managed/durable backends (durable dedup, managed memory, an A2A gateway) activated by capability negotiation, not code changes. Their tiers run from raw Apache Iggy up to the managed LaserData Cloud runtime.
