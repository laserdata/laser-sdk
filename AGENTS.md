...
sdk/src/
  ...
  agent/
    ...
    builder.rs        Agent::builder + AgentHandle (ready/shutdown/join/abort); capabilities +
                      ack_on_pickup + inbox_route, self-advertises card + presence on spawn
    ctx.rs            AgentCtx handed to a handler: respond/reply_on/send/request/respond_input,
                      fan_out (gather under GatherPolicy), approval_gate, spawn_subconversation
    ...
    router.rs         Router (To / Broadcast / ToCapable / AllCapable), CapabilitySelector,
                      RoutePolicy, InboxRoute (Advertised / Fixed)
    contract.rs       Laser::contract -> Contract (Completed/Failed/NotConsumed/TimedOut), the
                      directed-task state machine; Laser::scatter (the Laser-level gather)
    workflow.rs       Laser::workflow -> the engine: topo-ordered steps, budgets, verifier panels,
                      saga compensation, journal/replay/resume, all_capable scatter, .exclusive() fence
...
This crate is **not "all Phase 1."** The Phase 1 core (vanilla Iggy) is shipped: provenance + causality, reliable consumer, context seam, memory seam, `Agent` builder / router / session / state, plus the `respond_on` + `AgentCtx` handler seam. Isolation is an Iggy stream boundary (one connection drives any number of streams), not a per-message header. On top of it we landed, early and behind feature flags, several features the spec files as later / Tier B: opt-in warm dedup on restart, a semantic `VectorMemory` (in-memory, pluggable `Embedder`), a durable `KvMemory` (`Memory` over the managed KV store, feature `kv`), and the `a2a-bridge` feature (A2A JSON-RPC over agent topics). The `concierge` example runs on real Claude (`llm-anthropic`) or OpenAI (`llm-openai`).
...
The **multi-agent orchestration layer** is shipped, all conventions over the log (the substrate stays a log, coordination is client-side state machines over offsets/deadlines/leases/replies). Discovery: agents advertise a capability card (`Laser::publish_card`, auto-published by the builder when `capabilities` is set) and a live inbox (`AgentPresence` over `set_client_metadata`, query-gated), fused by `AgentRegistry` into one resolve-by-capability view that is health-aware (skips `Health::Unavailable`) and excludes quarantined agents (`Laser::quarantine`, an `OPERATION_QUARANTINE` registry fact; `Laser::unquarantine` lifts it, an `OPERATION_UNQUARANTINE` fact with the same authorization, so quarantine is reversible without waiting on retention expiry; both authorized by the topic write ACL). The registry is cached per data-stream on the connection: `Laser::agent_registry` seeds the folded cards, quarantine set, presence, and cursor offsets from a shared per-stream cache and resumes the fold instead of re-reading from offset 0, `refresh` folds only new records and evicts cards past their ttl (so the cached set stays bounded by live agents), and presence is reused within a ~2s ttl instead of paging the connection table every send. Presence carries the connection's authenticated `user_id` (a `PresenceEntry`): `inbox_for_principal(agent, user_id)` resolves the inbox only when it matches that principal (defense against a connection claiming another principal's agent id), while `inbox_for` stays the claim-only resolve. With the `sign` feature, `quarantine_signed`/`unquarantine_signed` carry an ed25519 signature and a registry built with `LaserBuilder::verifier(Arc<KeyRegistry>)` folds a quarantine/unquarantine fact only when its signature verifies; identity is a claim and the broker ACL stays the primary authorization, so this is defense in depth over it. Routing: `Router::{To,Broadcast,ToCapable,AllCapable}` + `InboxRoute::{Advertised,Fixed}` (resolve a target to its advertised inbox, never a hard-coded shared topic, the stream is the tenant boundary). Coordination: `Laser::contract` (a directed task -> `Contract::{Completed,Failed,NotConsumed,TimedOut}` with an ack-on-pickup `Working` signal, no producer offset), `AgentCtx::fan_out` + `Laser::scatter` (gather under `GatherPolicy::{RequireAll,Quorum,BestEffort}`), `AgentCtx::approval_gate` (pause on a human). The **workflow engine** (`Laser::workflow`): dependency-ordered steps (topological sort), `Budget` (tokens/wall-clock/invocations), `verify_with` panels, saga `compensate_with`, crash-recovery journal/replay/resume over `AgentTopic::WorkflowJournal`, `all_capable` scatter steps, and a per-step `.exclusive()` claiming a fenced lease (`acquire_fence`, kv-gated) that stamps the monotonic token via `ContractBuilder::fence`. An exclusive step pins a stable task conversation (derived from `run_id` + step label) via `ContractBuilder::conversation` so the fence gate buckets re-dispatches together and actually compares a stale holder's token against the new one; `acquire_fence` fails closed (`Unsupported`) when the server does not advertise the fenced compare-and-swap capability (`KV_CAS_FENCED`), so the SDK refuses to claim at-most-once on a server whose fence may not be monotonic. `StepHandle::on_timeout(OnTimeout::{Fail,Reassign})` (default `Fail`): `Reassign` (requires an exclusive step) hands a timed-out task to a fresh holder by re-acquiring the lease, which bumps the fence sequence so the new token strictly exceeds the old one and the timed-out holder is gated out, bounded to a few reassignments. The durable cross-holder at-most-once external effect is the handler's, via a fenced `Kv::cas_fenced` commit carrying the token; the engine provides the token and the same-holder replay gate. With a verifier enrolled the contract reply path verifies a reply's signature before accepting a terminal, so a forged reply echoing the correlation cannot short-circuit the contract. The reply reader seeds at the pre-send tail (no offset-0 history scans). The consumer's per-task fence high-water map is bounded (idle entries swept past a ttl once over a soft cap), so a long-lived consumer stays bounded by its active working set. Memory P4: `DefaultConsolidator` (mechanical prune) and `to_context_block` token-budgeted recall. All bound in the Python SDK (`Laser.contract`/`scatter`/`quarantine`, `spawn_agent` capabilities/ack_on_pickup/health, `AgentMessage.body()`), with the matched `orchestra` example (Rust + Python). Still on the plane side: HA leader election, the journal/graph row projections, the monotonic fence sequence the lease re-acquire and `OnTimeout::Reassign` rely on (and the `KV_CAS_FENCED` capability advertising it), and the quarantine/unquarantine registry projection; on the seam side, the `DefaultConsolidator` summarize + fact-invalidation halves (need a generation seam).
