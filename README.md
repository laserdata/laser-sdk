...
**Rust** is the reference SDK. **Python** ([`foreign/python`](foreign/python/README.md)) binds the same Rust core. Both are at `0.0.1-rc.18`. **TypeScript** ([`foreign/typescript`](foreign/typescript/README.md)) is a native Node client at `0.0.1-rc.2`, checked against the same fixture and BDD corpus. The standalone [`laser-wire`](wire/README.md) crate owns the language-neutral contract.
...
- [Examples](examples/README.md): aligned Rust, Python, and TypeScript systems runnable locally or against LaserData Cloud.
...
| [`examples`](examples/README.md) | nine runnable systems: a focused direct-streaming producer/consumer walkthrough, event analytics, an order book, a firehose load generator, an agentic support desk, an agentic-memory loop, an A2A/MCP/AG-UI interop gateway, the `orchestra` multi-agent orchestrator, and a governance scenario. |
