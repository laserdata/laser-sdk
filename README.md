...
try {
  const topic = laser.topic("inferences")
  await topic.ensure(4)
  await topic.send(new TextEncoder().encode('{"latency_ms":42}'))
} finally {
  await laser.close()
}
```
